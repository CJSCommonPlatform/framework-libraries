...
public class HasEventsMatcher extends TypeSafeDiagnosingMatcher<List<JsonNode>>{
    ...
    protected boolean matchesSafely(final List<JsonNode> actualEvents, final Description description) {
        ...
        } else if(actualEventsSize == 1) {
            description.appendText("found this event in stream:\n" + actualEvents.get(0));
        }
        else {
            description.appendText("found these events in stream:\n" + actualEvents);
        }
    ...
    public void describeTo(final Description description) {
        ...
        if(expectedEventsSize == 0) {
         ...
         } else if(expectedEventsSize == 1) {
                description.appendText("this event in stream:\n" + expectedEvents.get(0));
        }
         else {
            description.appendText("these " + expectedEventsSize + " events in stream:\n" + expectedEvents);
        }
    }
